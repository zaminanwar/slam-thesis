...
\small
Gazebo Harmonic $\xrightarrow{\texttt{/scan, /odom, /clock}}$ SLAM Algorithm $\xrightarrow{\texttt{map}\rightarrow\texttt{odom}~\text{TF}}$ Nav2 Stack $\xrightarrow{\texttt{/cmd\_vel}}$ Gazebo\\[4pt]
Gazebo $\xrightarrow{\texttt{/model/rover/pose}}$ GT Publisher $\xrightarrow{\texttt{map\_gt}\rightarrow\texttt{base\_footprint\_gt}~\text{TF}}$ Trajectory Exporter $\rightarrow$ \texttt{gt.tum}\\[4pt]
SLAM TF Tree $\rightarrow$ Trajectory Exporter $\rightarrow$ \texttt{est.tum} $\rightarrow$ \texttt{evo} $\rightarrow$ \texttt{metrics.json}
}}
